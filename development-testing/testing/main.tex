...
Given the straightforward nature of this product, the testing strategy is kept deliberately minimal. Instead, five practical tests are conducted to verify that all core functions operate correctly and that the device meets basic reliability and requirements.
...
Device temperature is monitored during prolonged operation. Under continuous use over an extended session, the surface temperature of all components remains below \textbf{45\,°C}, confirming that no active cooling is required and that the thermal performance of both the electronics and the 3D-printed PLA enclosure is within safe operating limits.